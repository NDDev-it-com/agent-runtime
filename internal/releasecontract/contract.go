...
func (c Contract) VerifyWorkflow(data []byte) error {
	...
	required := []string{
		...
		"--expect-version", `release_parent="$(mktemp -d "${RUNNER_TEMP}/agent-runtime-release.XXXXXX")"`, `release_dist="${release_parent}/release-dist"`, `--out "$release_dist"`, `RELEASE_DIST=$release_dist`, `${{ env.RELEASE_DIST }}`,
	}
