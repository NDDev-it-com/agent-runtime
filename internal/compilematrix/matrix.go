...
func copyDriver(source, destination string) (identity driverIdentity, rootErr error) {
	...
	defer func() { rootErr = errors.Join(rootErr, sourceFile.Close()) }()
	info, err := sourceFile.Stat()
	if err != nil || !info.Mode().IsRegular() { return identity, errors.New("source test driver is not one regular file") }
	destinationFile, err := os.OpenFile(destination, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o500)
	if err != nil { return identity, err }
	defer func() { rootErr = errors.Join(rootErr, destinationFile.Close()) }()
	hash := sha256.New()
	written, err := io.Copy(io.MultiWriter(destinationFile, hash), sourceFile)
	if err != nil || written != info.Size() { return identity, errors.New("copy test driver was incomplete") }
	if err := destinationFile.Sync(); err != nil { return identity, err }
	copy(identity.digest[:], hash.Sum(nil))
	identity.path, identity.size, identity.mode = destination, written, 0o500
	return identity, nil
}

func validateDriver(identity driverIdentity) error {
	info, err := os.Lstat(identity.path)
	if err != nil { return err }
	if !info.Mode().IsRegular() || info.Mode().Perm() != identity.mode || info.Size() != identity.size {
		return errors.New("owned test driver identity, type, mode, or size changed")
	}
	data, err := os.ReadFile(identity.path)
	if err != nil { return err }
	if sha256.Sum256(data) != identity.digest { return errors.New("owned test driver content changed") }
	return nil
}
